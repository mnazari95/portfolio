...
Next page should be similar to the screenshot above. Now choose the REST API type and click Build.
...
We now need to connect our api gateway endpoint to our lambda function. Lets 
head over to API Gateway services, select the api we made earlier and follow the instructions from the images below.
...
While you're at integration request page, make sure to select the lambda region that your lambda function is located in and save. Once we made this change we will need to re-enable CORS for our api and deploy(can be same stage) it. Feel free
